...
class Command(BaseCommand):
    def handle(self, *args, **options):
        plan, _ =Plans.objects.get_or_create(title="Junior", price=0, max_daily_participations=1, max_interview_participations=1, 
                             max_common_sentences=5, description="You already have this plan!", card_text_color="danger")
        plan, _ =Plans.objects.get_or_create(title="Mid-Level", price=7, max_daily_participations=3,
        ...
        plan, _ =Plans.objects.get_or_create(title="Senior", price=20, max_daily_participations=10,
